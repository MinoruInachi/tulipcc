...
Tulip には 3 種類のフォントが内蔵されています。
 ...
 - TFB フォント: TFB 用の固定サイズフォントを 3 つ同梱しています（後述）。`tulip.tfb_font()` で実行時に切り替えられます。
 ...
 - Tulip フォント: `bg_str` などで使うフォントを 19 種類同梱しています。`lv.tulip_font_13` のように参照すれば LVGL のウィジェットでも使えます。
...
TFB は 3 つの内蔵固定幅フォントに対応しており、`tulip.tfb_font(x)` で実行時に切り替えられます。
...
# TFB のフォント番号を設定・取得します（0=8x12、1=6x8、2=12x16）
tulip.tfb_font(x)
