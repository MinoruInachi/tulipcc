...
コンソールは行指向の端末です。色などの SGR 属性、`ESC[K`、`ESC[H`、`ESC[J`、`\r`、
`\b` を解釈するので、シェルのプロンプトや `ls`、`git` のように流れていく出力は
そのまま扱えます。一方、全画面プログラム（`vi`、`htop`、`tmux`）が使うカーソル移動、
スクロール領域、代替画面は未実装なので、それらはまだ正しく描画されません。
