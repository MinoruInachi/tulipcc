...
以降 `Ctrl-\` でキーボードを IME に渡し、また戻せます。JIS キーボードなら専用キーを
そこに割り当てられます（`boot.py` で）。

```python
tulip.key_remap(0x8a, 0, 0x1c)   # 変換キー
tulip.key_remap(0x35, 0, 0x1c)   # 半角/全角キー（JIS の位置）
```

IME がキーボードを持っている間、打った文字は行に直接入らず、コンソール最下行の
変換ストリップに出ます。確定した文字列だけがエディタ・LVGL のテキストエリア・REPL に
届きます。
...
カタカナとひらがなは常に最後の 2 候補として提示されるので、辞書に無いものも
space で候補を送れば入力できます。

```python
ime.start()                  # 有効化。辞書も読み込みます（約 2 秒）
ime.start(False)             # 辞書なしで有効化。かな入力のみ
