...
画面で見えている通りの値を返します。セッション中にそのサイズが変わった場合はアプリが
ウィンドウ変更を送ります。TFB のフォントを切り替えると桁数が変わり、古い幅のまま折り
返している相手は以降の行のレイアウトをすべて誤るからです。Tab5 では、セッションの
途中で日本語が来てもサイズは動きません。コンソールが切り替わる先はフォント `5`、
つまり 12x16 のコンソールに日本語を足したものであって、別のサイズではありません
（`tulip.tfb_font()` の項を参照）。
...
`sftp` は同じ仕組みの接続でファイルを転送します。話す相手はサーバーの SFTP
サブシステムで、`scp` のワイヤプロトコル（OpenSSH 自身が 9.0 以降離れつつあります）
ではありません。OpenSSH サーバーに最初から入っているもの以外、リモート側に必要な
ものはありません。

```python
import sftp

# 片方向ずつ 1 ファイルと、一覧。呼び出しごとに 1 接続です
...
# 複数ファイルを扱うなら接続を開いたまま使います
s = sftp.connect("192.168.1.10", "me", key="/user/id_rsa")
print(s.realpath("."))                    # 相対パスの起点
for name, longname, attrs in s.ls("."):   # longname はサーバー自身の ls -l の行
    print(longname)
s.get("notes.txt")                        # 名前はそのまま、カレントディレクトリへ
s.get("notes.txt", "/user/")              # ディレクトリ指定でも名前はそのまま
...
`get()` と `put()` は転送したバイト数を返し、`progress(done, total)` コールバックを
受け取ります。16KB ごとに呼ばれ、サーバーがサイズを返さなかった場合 `total` は
`None` です。サーバーが拒否したものは `sftp.SFTPError` になります。これは
`ssh.SSHError` の一種で、サーバー自身のメッセージと、`.code` にステータスコードを
持ちます。`except sftp.SFTPError as e: if e.code == sftp.FX_NO_SUCH_FILE` と書けば、
ファイルが無いのか拒否されたのかを区別できます。

Tab5 の Wi-Fi で実測すると、120KB のファイルがどちら向きでも 1〜2 秒で移ります。
受信は 62 KB/s 前後で安定し、送信は実行ごとに 65〜105 KB/s と振れますが、これは
コードではなく電波の差です。小さいファイルではファイル本体より接続のほうが高くつく
（鍵認証だとハンドシェイクに約 2 秒）ので、1 回で済ませる関数と並んで `connect()`
があります。
...
配置してあります。接続は Connect ボタンのほか、どのフィールドでも Enter で行えます。
オンスクリーンキーボードの改行キーも同じで、送信のためにキーボードをどける必要は
ありません。

Files ボタンを押すと、フォームが転送ページに変わります。リモートのパス、ローカルの
パス、そして Get・Put・List のボタンがあります。List はリモートのディレクトリを右の
枠に表示し、ディレクトリには名前のうしろに `/` を付けるので、これから入力する名前を
見ながら作業できます。転送はフォームのホスト・ユーザー・認証情報で独自の接続を張り
ます。入力したパスワードはアプリが動いているあいだ RAM に保持され（`sshterm.conf`
には書きません）、接続時にフィールドが消えたあとでも転送できます。2 つのパスも
ホストなどと一緒に設定ファイルに記憶されます。このページはフォームの一部なので、
シェルのセッションを始める前か、終えたあとに使う場所です。
...
**Tab5 では BG プレーンが RGB565（65,536 色）です。** 以下の API はいずれも従来どおり 0-255 のパレット番号を受け付け、パレット番号は他のボードと同じ色に描かれるので、パレット向けに書いたプログラムはそのまま動きます。加えて、色を取る引数（以下の `pal_idx` と、`tfb_str` の `fg` / `bg`）には 0-255 の `(r, g, b)` タプルも渡せ、その色を直接指定できます。0-255 の範囲外の整数はエラーになります。`bg_png` は PNG の色をそのまま保つので、事前に 255 色へ減色する必要はありません。`bg_pixel(x, y)` は従来どおり最も近いパレット番号を返し、`bg_pixel_rgb(x, y)` はピクセルをそのまま読み出します。`bg_bitmap` と `sprite_bitmap` の生バイト列は 1 ピクセル 1 バイトではなく 2 バイト（RGB565 リトルエンディアン）になり、`screenshot()` は RGB の PNG を書き出します。透明色はこれまでどおりパレット番号 `0x55` です。

```python
# Tab5 のみ: パレットと並行して使えるフルカラー指定
...
Tulip には 3 種類のフォントが内蔵されています。
 ...
 - TFB フォント: TFB 用の固定サイズフォントを 6 つ同梱しています（後述）。`tulip.tfb_font()` で実行時に切り替えられます。
...
TFB は 6 つの内蔵固定幅フォントに対応しており、`tulip.tfb_font(x)` で実行時に切り替えられます。
 ...
 - `5`: フォント `2` に日本語を足したもの — 半角セル 12x16、全角 24x16

日本語を表示できるのは `3`・`4`・`5` です。全角文字は TFB の 2 セルを占めるので、
...
`5` はフォント `2` のコンソールに日本語を足したものです。セルは 12x16 のまま、桁数も
そのままで、CP437 で表せるもの（英数字、罫線、アクセント付き文字）は引き続き 12x16
フォントから描かれます。つまり英文の行はフォント `2` と `5` でまったく同じに見えます。
日本語だけが日本語フォントから、本来の 16x16 のまま、2 セル分の 24x16 の枠の中央に
置かれます。1 列おきに引き伸ばして 24 ドット幅にはしません。全角の日本語は正方形が
前提で、1.5 倍に広げると漢字の縦画の一部だけが太くなってしまうからです。
描くときに太らせています。日本語フォントの線が 1px、隣に並ぶ 12x16 欧文が 2px なので、
そのままでは日本語だけ一段薄く見えるからです。太らせ方は、各ストロークを右へ 1px
伸ばす、ただしすぐ右隣に別のストロークがある場合は伸ばさない、というものです。16px の
込み入った漢字はストローク同士の間隔が 1px しかなく、そこを塗ってしまうと 電 や 器 が
塊になってしまいます。その代わり日本語の字間が少し広くなりますが、引き換えに、日本語を
表示し始めてもコンソールのサイズが変わらなくなります。

日本語を見るために手動でフォントを切り替える必要はありません。CP437 のフォントでは
どれも描けない文字を初めて表示しようとしたとき、コンソールは今のセルサイズに合う
日本語フォントへ自動で切り替わります。フォント `2` からはフォント `5`、それ以外からは
フォント `3` です。合うものがある場合は画面上の何も動きません。これが特に効くのが ssh
セッションで、相手には接続時に桁数を伝えてあるため、コンソールが黙って桁数を変えると
以降の行のレイアウトがすべて崩れてしまいます。自分で `tulip.tfb_font()` を呼ぶと、
そのセッションでは以降この自動切り替えは行われません（明示的に選んだフォントを勝手に
変えることはしません）。
...
# 5=フォント 2 の 12x16 セルで日本語
# 日本語フォントを含まないビルドのボードでは 3・4・5 は ValueError になります。
tulip.tfb_font(x)
font_num = tulip.tfb_font()

# 日本語は準備不要です。表示しようとした時点でコンソールが、今のセルサイズに合う
# 日本語フォントへ自動で切り替わります。
print("日本語と English が混在する行。ABCdefg 0123")
tulip.tfb_font(4)   # 同じフォントで 2 倍のサイズ。Tab5 では 80x22
tulip.tfb_font(5)   # フォント 2 と同じサイズで日本語。Tab5 では 106x45
...
マイクが動いている間は、その最新のブロックが AMY にも渡ります。`wave=amy.AUDIO_EXT0`
（左マイク）または `amy.AUDIO_EXT1`（右マイク）のオシレータを作ると、マイクが拾った音が
そのまま鳴ります。エンベロープもフィルタもエフェクトも、他のオシレータと同じように効き、
音量も同じく `amp`/`vel` に従います。（ESP32-S3 のボードでは AMY 自身が I2S から録った音声を
使いますが、Tab5 は音声出力を自前で行うため、代わりにサンプルを渡す形にしています。）

このオシレータを作るまでスピーカーには何も出ません。録音の邪魔にもならず、
`mic_read()` と `mic_record()` はこれまでどおり全フレームを受け取ります。

```python
tulip.mic_start()
amy.send(osc=200, wave=amy.AUDIO_EXT0, vel=1)   # 左マイクの音をそのまま
amy.send(osc=200, filter_freq=800, resonance=4, filter_type=amy.FILTER_LPF)
amy.send(osc=200, vel=0)                        # 素通しを止める
tulip.mic_stop()                                # オシレータは無音になる
```

**ハウリングに注意してください。** スピーカーはマイクから数センチしか離れていないので、
大きな音量で素通しすると発振します。`amp` は小さめにするか、ヘッドホンを使ってください。
実機で測ったところ、`amp=16` ですでに発振し始めました。
...
## モーションセンサー / IMU（Tab5 のみ）

Tab5 には Bosch BMI270 の 6 軸 IMU（加速度センサー＋ジャイロ）が内蔵されています。
`tulip.imu()` は 1 サンプルを読み取り、6 つの float を返します。

```python
ax, ay, az, gx, gy, gz = tulip.imu()   # 加速度は g、ジャイロは度/秒
```

加速度は g 単位（静止時は重力によりいずれかの軸が約 1.0 になるので、
`sqrt(ax*ax + ay*ay + az*az)` は約 1.0）、ジャイロは度/秒（静止時はほぼ 0）です。
センサーは初回呼び出し時に立ち上がり（加速度・ジャイロとも 100 Hz、レンジは
±4 g と ±2000 dps）、以降は要求時にのみポーリングします。つまりサンプルを要求
するまで何も動きません。`frame_callback` などから好きなだけ読んで、傾きや動きの
検出に使えます。

```python
import math
ax, ay, az, _, _, _ = tulip.imu()
# 水平からの画面の傾き（ピッチ角、度）の概算
